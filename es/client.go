...
func NewClientWithSchema(schema string) (*Client, error) {
	...
	err := uconfig.LoadConfToStruct("Es."+schema, &conf)
	if err != nil {
		ulog.SyslogError("Es-NewClient", fmt.Sprintf("查询Es[%s]配置失败%s", schema, err.Error()))
		return nil, err
	}
	newClient, err := elasticsearch.NewClient(conf)
	if err != nil {
		ulog.SyslogError("Es-NewClient", fmt.Sprintf("连接Es服务[%v]服务失败%s", conf.Addresses, err.Error()))
		return nil, err
	}
